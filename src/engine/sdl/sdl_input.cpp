...
static q3SDL_Keysym IN_MakeKeysym( const SDL_KeyboardEvent *event )
{
	q3SDL_Keysym keysym;

	keysym.scancode = event->scancode;
	keysym.sym = event->key;
	keysym.mod = event->mod;

	return keysym;
}

static void IN_SetCursorVisible( qboolean visible )
{
	if ( visible )
		SDL_ShowCursor();
	else
...
static void IN_SetWindowMouseGrabbed( qboolean grabbed )
{
	SDL_SetWindowMouseGrab( SDL_window, grabbed ? true : false );
}
...
===============
*/
static void IN_PrintKey( const q3SDL_Keysym *keysym, keyNum_t key, qboolean down )
{
	if( down )
		Com_Printf( "+ " );
	else
...
static constexpr keyNum_t KeyNumValue( int value ) noexcept
{
	return static_cast<keyNum_t>( value );
}
...
static qboolean IN_IsConsoleKey( keyNum_t key, int character )
{
	...
	if ( cl_consoleKeys->modified )
	{
		...
		while( numConsoleKeys < MAX_CONSOLE_KEYS )
		{
			...
			{
				const int quakeKey = Key_StringToKeynum( token );

				c->type = consoleKey_t::QUAKE_KEY;
				c->u.key = KeyNumValue( quakeKey );
...
===============
*/
static keyNum_t IN_TranslateSDLToQ3Key( q3SDL_Keysym *keysym, qboolean down )
{
	keyNum_t key = KeyNumValue( 0 );
	...
	if ( keysym->scancode >= SDL_SCANCODE_1 && keysym->scancode <= SDL_SCANCODE_0 )
	{
		...
		if( keysym->scancode == SDL_SCANCODE_0 )
			...
			key = KeyNumValue( '1' + keysym->scancode - SDL_SCANCODE_1 );
	}
	else if ( in_forceCharset->integer > 0 )
	{
		if ( keysym->scancode >= SDL_SCANCODE_A && keysym->scancode <= SDL_SCANCODE_Z )
		{
			key = KeyNumValue( 'a' + keysym->scancode - SDL_SCANCODE_A );
		}
		else
		{
			switch ( keysym->scancode )
			{
				case SDL_SCANCODE_MINUS:        key = KeyNumValue( '-' );  break;
				case SDL_SCANCODE_EQUALS:       key = KeyNumValue( '=' );  break;
				case SDL_SCANCODE_LEFTBRACKET:  key = KeyNumValue( '[' );  break;
				case SDL_SCANCODE_RIGHTBRACKET: key = KeyNumValue( ']' );  break;
				case SDL_SCANCODE_NONUSBACKSLASH:
				case SDL_SCANCODE_BACKSLASH:    key = KeyNumValue( '\\' ); break;
				case SDL_SCANCODE_SEMICOLON:    key = KeyNumValue( ';' );  break;
				case SDL_SCANCODE_APOSTROPHE:   key = KeyNumValue( '\'' ); break;
				case SDL_SCANCODE_COMMA:        key = KeyNumValue( ',' );  break;
				case SDL_SCANCODE_PERIOD:       key = KeyNumValue( '.' );  break;
				case SDL_SCANCODE_SLASH:        key = KeyNumValue( '/' );  break;
				default:
					/* key = 0 */
					break;
			}
	...
	if( !key && keysym->sym >= SDLK_SPACE && keysym->sym < SDLK_DELETE )
	{
		...
		key = KeyNumValue( static_cast<int>( keysym->sym ) );
	}
	else if( !key )
	{
		switch( keysym->sym )
		{
			...
			case SDLK_KP_MULTIPLY:  key = KeyNumValue( '*' ); /*K_KP_STAR;*/ break;
...
static void IN_ActivateMouse( void )
{
	...
	if ( !mouseActive )
	{
		...
		IN_SetWindowMouseGrabbed( qtrue );

		if ( glw_state.isFullscreen )
			IN_SetCursorVisible( qfalse );
	...
	if ( !glw_state.isFullscreen )
	{
		if ( in_nograb->modified || !mouseActive )
		{
			if ( in_nograb->integer ) {
				...
				IN_SetWindowMouseGrabbed( qfalse );
			} else {
				SDL_SetWindowRelativeMouseMode( SDL_window, in_mouse->integer == 1 );
				IN_SetWindowMouseGrabbed( qtrue );
			}
...
static void IN_InitJoystick( void )
{
	...
	for (i = 0; i < total; i++)
	{
		...
		Q_strcat(buf, sizeof(buf), name ? name : "<unnamed joystick>");
	...
	selected = joysticks[in_joystickNo->integer];
	stick = SDL_OpenJoystick( selected );
	...
	stickInstance = selected;

	if (SDL_IsGamepad(selected))
...
void HandleEvents( void )
{
	...
	q3SDL_Keysym keysym;
	keyNum_t key = KeyNumValue( 0 );
	static keyNum_t lastKeyDown = KeyNumValue( 0 );
		...
		switch( e.type )
		{
			case SDL_EVENT_KEY_DOWN:
				if ( e.key.repeat && Key_GetCatcher() == 0 )
					break;
				keysym = IN_MakeKeysym( &e.key );
				key = IN_TranslateSDLToQ3Key( &keysym, qtrue );
			...
			case SDL_EVENT_TEXT_INPUT:
				if( lastKeyDown != K_CONSOLE )
				{
					...
					while ( *c )
					{
						...
						if( utf32 != 0 )
						{
							if ( IN_IsConsoleKey( KeyNumValue( 0 ), utf32 ) )
							{
			...
			case SDL_EVENT_MOUSE_MOTION:
				if( mouseActive )
				{
					const int dx = e.motion.xrel >= 0.0f ? (int)( e.motion.xrel + 0.5f ) : (int)( e.motion.xrel - 0.5f );
					const int dy = e.motion.yrel >= 0.0f ? (int)( e.motion.yrel + 0.5f ) : (int)( e.motion.yrel - 0.5f );
			...
			case SDL_EVENT_MOUSE_BUTTON_UP:
				{
					...
					Com_QueueEvent( in_eventTime, SE_KEY, b,
						e.button.down ? qtrue : qfalse, 0, NULL );
				}
				break;

			case SDL_EVENT_MOUSE_WHEEL:
				{
					int wheelY = e.wheel.integer_y;
...
#ifdef DEBUG_EVENTS
				Com_Printf( "%4llu %s\n", (unsigned long long)e.window.timestamp, eventName( e.type ) );
#endif
				switch ( e.type )
				{
					case SDL_EVENT_WINDOW_MOVED:
					...
					// keyboard focus:
					case SDL_EVENT_WINDOW_FOCUS_LOST:	lastKeyDown = KeyNumValue( 0 ); Key_ClearStates(); IN_SyncModifiers(); gw_active = qfalse; break;
					case SDL_EVENT_WINDOW_FOCUS_GAINED:	lastKeyDown = KeyNumValue( 0 ); Key_ClearStates(); IN_SyncModifiers(); gw_active = qtrue; gw_minimized = qfalse;
