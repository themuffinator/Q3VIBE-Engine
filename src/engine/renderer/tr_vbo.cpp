...
void R_BuildWorldVBO( msurface_t *surf, int surfCount )
{
	...
	// 0 item is unused
	vbo->items = Q_AllocPtr( RI_HUNK_ALLOC_CALL( ( numStaticSurfaces + 1 ) * sizeof( vbo_item_t ), h_low ) );
	vbo->items_count = numStaticSurfaces;

	// last item will be used for run length termination
	vbo->items_queue = Q_AllocPtr( RI_HUNK_ALLOC_CALL( ( numStaticSurfaces + 1 ) * sizeof( int ), h_low ) );
	...
	// vertex buffer
	vbo->vbo_buffer = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( vbo_size ) );
	vbo->vbo_offset = 0;
	vbo->vbo_size = vbo_size;

	// index buffer
	vbo->ibo_buffer = Q_AllocPtr( RI_HUNK_ALLOC_CALL( ibo_size, h_low ) );	
	vbo->ibo_offset = 0;
	vbo->ibo_size = ibo_size;

	// soft index buffer
	vbo->soft_buffer = Q_AllocPtr( RI_HUNK_ALLOC_CALL( ibo_size, h_low ) );
	vbo->soft_buffer_indexes = 0;

	// ibo runs buffer
	vbo->ibo_items = Q_AllocPtr( RI_HUNK_ALLOC_CALL( ( (numStaticIndexes / MIN_IBO_RUN) + 1 ) * sizeof( ibo_item_t ), h_low ) );
	vbo->ibo_items_count = 0;

	surfList = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( numStaticSurfaces * sizeof( msurface_t * ) ) );
