...
static void R_LoadMergedLightmaps( const lump_t *l, byte *image )
{
	...
	tr.lightmaps = Q_AllocPtr( RI_HUNK_ALLOC_CALL( tr.numLightmaps * sizeof(image_t *), h_low ) );
...
static void R_LoadVisibility( const lump_t *l ) {
	...
	s_worldData.novis = Q_AllocPtr( RI_HUNK_ALLOC_CALL( len, h_low ) );
	...
	if ( tr.externalVisData ) {
		...
		dest = Q_AllocPtr( RI_HUNK_ALLOC_CALL( len - 8, h_low ) );
...
static void GenerateNormals( srfSurfaceFace_t *face )
{
	...
	face->normals = Q_AllocPtr( RI_HUNK_ALLOC_CALL( face->numPoints * sizeof( tess.normal[0] ), h_low ) );
...
static void ParseFace( const dsurface_t *ds, const drawVert_t *verts, msurface_t *surf, int *indexes ) {
	...
	cv = Q_AllocPtr( RI_HUNK_ALLOC_CALL( sfaceSize, h_low ) );
...
static void ParseTriSurf( const dsurface_t *ds, const drawVert_t *verts, msurface_t *surf, int *indexes ) {
	...
	tri = Q_AllocPtr( RI_HUNK_ALLOC_CALL( sizeof( *tri ) + numVerts * sizeof( tri->verts[0] ) 
...
static void ParseFlare( const dsurface_t *ds, const drawVert_t *verts, msurface_t *surf, int *indexes ) {
	...
	flare = Q_AllocPtr( RI_HUNK_ALLOC_CALL( sizeof( *flare ), h_low ) );
...
static void R_MovePatchSurfacesToHunk( void ) {
	...
	for ( i = 0; i < s_worldData.numsurfaces; i++ ) {
		...
		hunkgrid = Q_AllocPtr( RI_HUNK_ALLOC_CALL( size, h_low ) );
		Com_Memcpy(hunkgrid, grid, size);

		hunkgrid->widthLodError = Q_AllocPtr( RI_HUNK_ALLOC_CALL( grid->width * 4, h_low ) );
		Com_Memcpy( hunkgrid->widthLodError, grid->widthLodError, grid->width * 4 );

		hunkgrid->heightLodError = Q_AllocPtr( RI_HUNK_ALLOC_CALL( grid->height * 4, h_low ) );
...
static void R_LoadSurfaces( const lump_t *surfs, const lump_t *verts, const lump_t *indexLump ) {
	...
	in = reinterpret_cast<const dsurface_t *>( fileBase + surfs->fileofs );
	if (surfs->filelen % sizeof(*in))
		ri.Error( ERR_DROP, "%s(): funny lump size in %s", __func__, s_worldData.name );
	count = surfs->filelen / sizeof(*in);

	dv = reinterpret_cast<const drawVert_t *>( fileBase + verts->fileofs );
	...
	out = Q_AllocPtr( RI_HUNK_ALLOC_CALL( count * sizeof(*out), h_low ) );
...
static void R_LoadSubmodels( const lump_t *l ) {
	...
	in = reinterpret_cast<const dmodel_t *>( fileBase + l->fileofs );
	if (l->filelen % sizeof(*in))
		ri.Error( ERR_DROP, "%s(): funny lump size in %s", __func__, s_worldData.name );
	count = l->filelen / sizeof(*in);

	s_worldData.bmodels = out = Q_AllocPtr( RI_HUNK_ALLOC_CALL( count * sizeof(*out), h_low ) );
...
static void R_LoadNodesAndLeafs( const lump_t *nodeLump, const lump_t *leafLump ) {
	...
	in = reinterpret_cast<const dnode_t *>( fileBase + nodeLump->fileofs );
	...
	out = Q_AllocPtr( RI_HUNK_ALLOC_CALL( (numNodes + numLeafs) * sizeof(*out), h_low ) );	
...
static	void R_LoadPlanes( const lump_t *l ) {
	...
	if (l->filelen % sizeof(*in))
		ri.Error( ERR_DROP, "%s(): funny lump size in %s", __func__, s_worldData.name );
	count = l->filelen / sizeof(*in);
	out = Q_AllocPtr( RI_HUNK_ALLOC_CALL( count*2*sizeof(*out), h_low ) );
...
static void R_LoadFogs( const lump_t *l, const lump_t *brushesLump, const lump_t *sidesLump ) {
	...
	s_worldData.fogs = Q_AllocPtr( RI_HUNK_ALLOC_CALL( s_worldData.numfogs*sizeof(*out), h_low) );
...
static void R_LoadLightGrid( const lump_t *l ) {
	...
	w->lightGridData = Q_AllocPtr( RI_HUNK_ALLOC_CALL( l->filelen, h_low ) );
...
static void R_LoadEntities( const lump_t *l ) {
	...
	w->entityString = Q_AllocPtr( RI_HUNK_ALLOC_CALL( l->filelen + 1, h_low ) );
...
void RE_LoadWorldMap( const char *name ) {
	...
	startMarker = static_cast<byte *>( RI_HUNK_ALLOC_CALL( 0, h_low ) );
	c_gridVerts = 0;
	...
	s_worldData.dataSize = static_cast<byte *>( RI_HUNK_ALLOC_CALL( 0, h_low ) ) - startMarker;
