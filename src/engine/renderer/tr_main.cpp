...
static void R_SetupFrustum( viewParms_t *dest, float xmin, float xmax, float ymax, float zProj, float stereoSep )
{
	...
	// near clipping plane
	VectorCopy( dest->orientation.axis[0], dest->frustum[4].normal );
...
void R_AddLitSurf( void *surface, shader_t *shader, int fogIndex )
{
	...
	litsurf->surface = static_cast<surfaceType_t *>( surface );
...
void R_AddDrawSurf( void *surface, shader_t *shader, 
	...
	tr.refdef.drawSurfs[index].surface = static_cast<surfaceType_t *>( surface );
...
static void R_SortDrawSurfs( drawSurf_t *drawSurfs, int numDrawSurfs ) {
	...
	for ( i = 0 ; i < numDrawSurfs ; i++ ) {
		...
		if ( shader->sort > ShaderSortValue( SS_PORTAL ) ) {
			break;
		}
