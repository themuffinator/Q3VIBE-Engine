...
static void ARB_Lighting( const shaderStage_t* pStage )
{
	...
	if ( tess.shader->sort < ShaderSortValue( SS_OPAQUE ) ) {
...
void ARB_SetupLightParams( void )
{
	...
	if ( fogPass ) {
		...
		if ( fp->eyeOutside ) {
			vertexProgram = static_cast<programNum>( vertexProgram + 2 );
		} else {
			vertexProgram = static_cast<programNum>( vertexProgram + 1 );
		}
		fragmentProgram = static_cast<programNum>( fragmentProgram + 1 );
	}
