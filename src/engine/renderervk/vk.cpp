...
static void vk_create_render_passes( void )
{
	...
	attachments[1].samples = static_cast<VkSampleCountFlagBits>( vkSamples );
	...
	if ( vk.msaaActive )
	{
		...
		attachments[2].samples = static_cast<VkSampleCountFlagBits>( vkSamples );
#ifdef USE_BUFFER_CLEAR
	...
	attachments[1].flags = 0;
	attachments[1].format = depth_format;
	attachments[1].samples = static_cast<VkSampleCountFlagBits>( vk.screenMapSamples );
	attachments[1].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR; // Need empty depth buffer before use
	attachments[1].storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
	...
	if ( vk.screenMapSamples > VK_SAMPLE_COUNT_1_BIT ) {
		...
		attachments[2].flags = 0;
		attachments[2].format = vk.color_format;
		attachments[2].samples = static_cast<VkSampleCountFlagBits>( vk.screenMapSamples );
#ifdef USE_BUFFER_CLEAR
		attachments[2].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
#else
		attachments[2].loadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
...
#define INIT_INSTANCE_FUNCTION(func) \
	q##func = reinterpret_cast<decltype(q##func)>( ri.VK_GetInstanceProcAddr( vk_instance, #func ) ); \
	if (q##func == NULL) {											\
...
static void vk_create_attachments( void )
{
	...
	if ( vk.fboActive ) {
		...
		// screenmap-msaa
		if ( vk.screenMapSamples > VK_SAMPLE_COUNT_1_BIT ) {
			create_color_attachment( vk.screenMapWidth, vk.screenMapHeight, static_cast<VkSampleCountFlagBits>( vk.screenMapSamples ), vk.color_format,
		...
		// screenmap depth
		create_depth_attachment( vk.screenMapWidth, vk.screenMapHeight, static_cast<VkSampleCountFlagBits>( vk.screenMapSamples ), &vk.screenMap.depth_image, &vk.screenMap.depth_image_view, qtrue );

		if ( vk.msaaActive ) {
			create_color_attachment( glConfig.vidWidth, glConfig.vidHeight, static_cast<VkSampleCountFlagBits>( vkSamples ), vk.color_format,
	...
	create_depth_attachment( glConfig.vidWidth, glConfig.vidHeight, static_cast<VkSampleCountFlagBits>( vkSamples ), &vk.depth_image, &vk.depth_image_view,
...
static byte *resample_image_data( const int target_format, byte *data, const int data_size, int *bytes_per_pixel )
{
	...
	case VK_FORMAT_B4G4R4A4_UNORM_PACK16:
		buffer = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( data_size / 2 ) );
		p = (uint16_t*)buffer;
	...
	case VK_FORMAT_B8G8R8A8_UNORM:
		buffer = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( data_size ) );
	...
	case VK_FORMAT_R8G8B8_UNORM: {
		buffer = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( (data_size * 3) / 4 ) );
...
void vk_create_post_process_pipeline( int program_index, uint32_t width, uint32_t height )
{
	...
	switch ( program_index ) {
		...
		case 2: // final bloom blend
			...
			samples = static_cast<VkSampleCountFlagBits>( vkSamples );
...
VkPipeline create_pipeline( const Vk_Pipeline_Def *def, renderPass_t renderPassIndex, uint32_t def_index ) {
	...
	multisample_state.rasterizationSamples = static_cast<VkSampleCountFlagBits>(
		( renderPassIndex == RENDER_PASS_SCREENMAP ) ? vk.screenMapSamples : vkSamples );
