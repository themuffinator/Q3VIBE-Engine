...
static void R_InitExtensions( void )
{
	...
	if ( R_HaveExtension( "GL_ARB_multitexture" ) )
	{
		...
		{
			qglMultiTexCoord2fARB = reinterpret_cast<decltype( qglMultiTexCoord2fARB )>( ri.GL_GetProcAddress( "glMultiTexCoord2fARB" ) );
			qglActiveTextureARB = reinterpret_cast<decltype( qglActiveTextureARB )>( ri.GL_GetProcAddress( "glActiveTextureARB" ) );
			qglClientActiveTextureARB = reinterpret_cast<decltype( qglClientActiveTextureARB )>( ri.GL_GetProcAddress( "glClientActiveTextureARB" ) );
	...
	if ( R_HaveExtension( "GL_EXT_compiled_vertex_array" ) )
	{
		if ( r_ext_compiled_vertex_array->integer )
		{
			...
			qglLockArraysEXT = reinterpret_cast<decltype( qglLockArraysEXT )>( ri.GL_GetProcAddress( "glLockArraysEXT" ) );
			qglUnlockArraysEXT = reinterpret_cast<decltype( qglUnlockArraysEXT )>( ri.GL_GetProcAddress( "glUnlockArraysEXT" ) );
...
static byte *RB_ReadPixels(int x, int y, int width, int height, size_t *offset, int *padlen, int lineAlign )
{
	...
	buffer = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( width * height * 4 + *offset + bufAlign - 1 ) );
	...
	*padlen = PAD(linelen, packAlign) - linelen;
	...
	// Allocate a few more bytes so that we can choose an alignment we like
	buffer = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( padwidth * height + *offset + bufAlign - 1 ) );
	bufstart = PADP((intptr_t) buffer + *offset, bufAlign);
...
static void R_LevelShot( void ) {
	...
	buffer = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( 128 * 128*3 + 18 ) );
...
static void GL_SetDefaultState( void )
{
	...
	glState.faceCulling = static_cast<cullType_t>( -1 );
...
void R_Init( void ) {
	...
	ptr = Q_AllocPtr( RI_HUNK_ALLOC_CALL( sizeof( *backEndData ) + sizeof(srfPoly_t) * max_polys + sizeof(polyVert_t) * max_polyverts, h_low) );
