...
static qboolean ParseStage( shaderStage_t *stage, const char **text )
{
	...
	if ( depthMaskExplicit && shader.sort == ShaderSortValue( SS_BAD ) ) {
		...
		if ( blendSrcBits == GLS_SRCBLEND_SRC_ALPHA && blendDstBits == GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA /*&& stage->rgbGen == CGEN_VERTEX*/ ) {
			...
			shader.sort = shader.polygonOffset ? ShaderSortValue( SS_DECAL ) : ShaderSortValue( SS_OPAQUE ) + 0.01f;
...
static void FindLightingStage( const int stage ) {
	...
	if ( shader.isSky || (shader.surfaceFlags & (SURF_NODLIGHT | SURF_SKY)) /* || shader.sort == ShaderSortValue( SS_ENVIRONMENT ) || shader.sort >= ShaderSortValue( SS_FOG ) */ ) {
		return;
	}
...
static void FindLightingBundle( void )
{
	...
	if ( /*shader.isSky || (shader.surfaceFlags & (SURF_SKY)) || */ shader.sort == ShaderSortValue( SS_ENVIRONMENT ) || shader.sort >= ShaderSortValue( SS_FOG ) ) {
		return;
	}
...
static shader_t *GeneratePermanentShader( void ) {
	...
	newShader = Q_AllocPtr( RI_HUNK_ALLOC_CALL( sizeof( shader_t ), h_low ) );
	...
	*newShader = shader;
	...
	for ( i = 0 ; i < newShader->numUnfoggedPasses ; i++ ) {
		...
		newShader->stages[i] = Q_AllocPtr( RI_HUNK_ALLOC_CALL( sizeof( stages[i] ), h_low ) );
		*newShader->stages[i] = stages[i];

		for ( b = 0 ; b < NUM_TEXTURE_BUNDLES ; b++ ) {
			size = newShader->stages[i]->bundle[b].numTexMods * sizeof( texModInfo_t );
			if ( size ) {
				newShader->stages[i]->bundle[b].texMods = Q_AllocPtr( RI_HUNK_ALLOC_CALL( size, h_low ) );
...
static void VertexLightingCollapse( void ) {
	...
	if ( shader.sort == ShaderSortValue( SS_OPAQUE ) ) {
...
static shader_t *FinishShader( void ) {
	...
	// set polygon offset
	//
	if ( shader.polygonOffset && shader.sort == ShaderSortValue( SS_BAD ) ) {
		shader.sort = SS_DECAL;
	}
	...
	if ( shader.sort == ShaderSortValue( SS_BAD ) ) {
		if ( colorBlend ) {
		...
		for ( i = 0; i < stage; i++ ) {
			...
			if ( env_mask == 1 && !pStage->depthFragment ) {
				if ( def.shader_type >= TYPE_GENERIC_BEGIN && def.shader_type <= TYPE_GENERIC_END ) {
					def.shader_type = static_cast<Vk_Shader_Type>( def.shader_type + 1 ); // switch to *_ENV version
...
static void ScanAndLoadShaderFiles( void )
{
	...
	s_shaderText = Q_AllocPtr( RI_HUNK_ALLOC_CALL( sum + numShaderxFiles * 2 + numShaderFiles * 2 + 1, h_low ) );
	...
	hashMem = Q_AllocPtr( RI_HUNK_ALLOC_CALL( size * sizeof(char *), h_low ) );
