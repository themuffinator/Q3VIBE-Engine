...
static void R_MipMap2( unsigned * const out, unsigned * const in, int inWidth, int inHeight ) {
	...
	if ( out == in )
		temp = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( outWidth * outHeight * 4 ) );
	else
		temp = out;
...
static void generate_image_upload_data( image_t *image, byte *data, Image_Upload_Data *upload_data ) {
	...
	upload_data->buffer = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( 2 * 4 * scaled_width * scaled_height ) );
	if ( data == NULL ) {
		Com_Memset( upload_data->buffer, 0, 2 * 4 * scaled_width * scaled_height );
	}

	if ( ( scaled_width != width || scaled_height != height ) && data ) {
		resampled_buffer = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( scaled_width * scaled_height * 4 ) );
	...
	scaled_buffer = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( sizeof( unsigned ) * scaled_width * scaled_height ) );
...
static void Upload32( byte *data, int x, int y, int width, int height, image_t *image, qboolean subImage )
{
	...
	if ( scaled_width != width || scaled_height != height ) {
		if ( data ) {
			resampledBuffer = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( scaled_width * scaled_height * 4 ) );
...
image_t *R_CreateImage( const char *name, const char *name2, byte *pic, int width, int height, imgFlags_t flags ) {
	...
	image = Q_AllocPtr( RI_HUNK_ALLOC_CALL( sizeof( *image ) + namelen + namelen2, h_low ) );
...
static void R_CreateFogImage( void ) {
	...
	data = Q_AllocPtr( (ri.Hunk_AllocateTempMemory)( FOG_S * FOG_T * 4 ) );
...
qhandle_t RE_RegisterSkin( const char *name ) {
	...
	tr.numSkins++;
	skin = Q_AllocPtr( RI_HUNK_ALLOC_CALL( sizeof( skin_t ), h_low ) );
	tr.skins[hSkin] = skin;
	...
	if ( strcmp( name + strlen( name ) - 5, ".skin" ) ) {
		skin->numSurfaces = 1;
		skin->surfaces = Q_AllocPtr( RI_HUNK_ALLOC_CALL( sizeof( skinSurface_t ), h_low ) );
	...
	skin->surfaces = Q_AllocPtr( RI_HUNK_ALLOC_CALL( skin->numSurfaces * sizeof( skinSurface_t ), h_low ) );
...
void	R_InitSkins( void ) {
	...
	skin = tr.skins[0] = Q_AllocPtr( RI_HUNK_ALLOC_CALL( sizeof( skin_t ), h_low ) );
