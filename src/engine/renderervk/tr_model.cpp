...
model_t *R_AllocModel( void ) {
	...
	mod = Q_AllocPtr( RI_HUNK_ALLOC_CALL( sizeof( *tr.models[tr.numModels] ), h_low ) );
...
static qboolean R_LoadMD3( model_t *mod, int lod, void *buffer, int fileSize, const char *mod_name ) {
	...
	mod->type = MOD_MESH;
	mod->dataSize += size;
	mod->md3[lod] = Q_AllocPtr( RI_HUNK_ALLOC_CALL( size, h_low ) );
...
static qboolean R_LoadMDR( model_t *mod, void *buffer, int filesize, const char *mod_name ) 
{
	...
	mod->dataSize += size;
	mod->modelData = mdr = Q_AllocPtr( RI_HUNK_ALLOC_CALL( size, h_low ) );
