...
// Best-effort tty output avoids warn_unused_result noise and handles EINTR.
static void Sys_WriteAll( int fd, const void *buffer, size_t length )
{
...
{
	Sys_WriteAll( STDOUT_FILENO, "\b \b", 3 );
}
...
void tty_Show( void )
{
	...
	if ( ttycon_hide > 0 )
	{
		...
		if ( ttycon_hide == 0 )
		{
			Sys_WriteAll( STDOUT_FILENO, "]", 1 ); // -EC-

			if ( tty_con.cursor > 0 )
			{
				Sys_WriteAll( STDOUT_FILENO, tty_con.buffer, tty_con.cursor );
			}
		}
	}
}
	...
	if ( ttycon_on )
	{
		...
		if (avail != -1)
		{
			...
			if (key && key < ' ')
			{
				...
				{
					Sys_WriteAll( STDOUT_FILENO, "\ec]", 3 );
					if ( tty_con.cursor )
					{
						Sys_WriteAll( STDOUT_FILENO, tty_con.buffer, tty_con.cursor );
					}
			...
			Sys_WriteAll( STDOUT_FILENO, &key, 1 );
		}
		return NULL;
	}
...
void Sys_Print( const char *msg )
{
	...
	Sys_WriteAll( STDERR_FILENO, printmsg, len );
...
int main( int argc, const char* argv[] )
{
	...
	cmdline = static_cast<char *>( malloc( len ) );
	*cmdline = '\0';
	for ( i = 1; i < argc; i++ )
	{
		if ( i > 1 )
