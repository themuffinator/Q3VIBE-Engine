...
FT_Bitmap *R_RenderGlyph(FT_GlyphSlot glyph, glyphInfo_t* glyphOut) {
	...
	if ( glyph->format == ft_glyph_format_outline ) {
		...
		bit2 = Q_AllocPtr( ri.Malloc( sizeof( FT_Bitmap ) ) );
		...
		bit2->buffer     = Q_AllocPtr( ri.Malloc( pitch * height ) );
		bit2->num_grays = 256;
...
static void WriteTGA (const char *filename, byte *data, int width, int height) {
	...
	buffer = Q_AllocPtr( ri.Malloc( width * height * 4 + 18 ) );
	...
	// flip upside down
	flip = Q_AllocPtr( ri.Malloc( width * 4 ) );
...
//		Com_Memcpy(font, faceData, sizeof(fontInfo_t));
	...
	out = Q_AllocPtr( ri.Malloc( 256 * 256 ) );
	if (out == NULL) {
	...
	while ( i <= GLYPH_END + 1 ) {
		...
		if (xOut == -1 || yOut == -1)  {
			...
			imageBuff = Q_AllocPtr( ri.Malloc( newSize ) );
			left = 0;
			max = 0;
