...
static struct BufferedFile *ReadBufferedFile(const char *name)
{
	...
	BF = Q_AllocPtr( ri.Malloc( sizeof( struct BufferedFile ) ) );
	if(!BF)
	{
		return(NULL);
	}
...
static q_alloc_ptr_t BufferedFileRead(struct BufferedFile *BF, unsigned Length)
{
	void *RetVal;
...
static uint32_t DecompressIDATs(struct BufferedFile *BF, uint8_t **Buffer)
{
	...
	*Buffer = DecompressedData;
	...
	CompressedData = Q_AllocPtr( ri.Malloc( CompressedDataLength ) );
	if(!CompressedData)
	{
		return((unsigned)-1);
	}
	...
	DecompressedData = Q_AllocPtr( ri.Malloc( puffDestLen ) );
	if(!DecompressedData)
	{
		ri.Free(CompressedData);
...
void R_LoadPNG(const char *name, byte **pic, int *width, int *height)
{
	...
	*pic = NULL;
	...
	OutBuffer = Q_AllocPtr( ri.Malloc( IHDR_Width * IHDR_Height * Q3IMAGE_BYTESPERPIXEL ) ); 
	if(!OutBuffer)
	{
		ri.Free(DecompressedData); 
		CloseBufferedFile(ThePNG);
