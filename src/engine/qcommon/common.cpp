...
#ifdef __cplusplus
#undef Hunk_Alloc
#undef Hunk_AllocateTempMemory
#undef Z_TagMalloc
#undef Z_Malloc
#undef S_Malloc

#ifdef ZONE_DEBUG
#define COM_Z_MALLOC(size_expr) Q_AllocPtr( Z_MallocDebug( (size_expr), const_cast<char *>( #size_expr ), const_cast<char *>( __FILE__ ), __LINE__ ) )
#define COM_S_MALLOC(size_expr) Q_AllocPtr( S_MallocDebug( (size_expr), const_cast<char *>( #size_expr ), const_cast<char *>( __FILE__ ), __LINE__ ) )
#else
#define COM_Z_MALLOC(size_expr) Q_AllocPtr( Z_Malloc( size_expr ) )
#define COM_S_MALLOC(size_expr) Q_AllocPtr( S_Malloc( size_expr ) )
#endif
#endif
...
#endif
	out = static_cast<char *>( COM_S_MALLOC( strlen( in ) + 1 ) );
	strcpy( out, in );
	return out;
}
...
static void Com_InitZoneMemory( void ) {
	...
	mainzone = static_cast<memzone_t *>( calloc( mainZoneSize, 1 ) );
	if ( !mainzone ) {
...
static void Com_InitHunkMemory( void ) {
	...
	s_hunkData = static_cast<byte *>( calloc( s_hunkTotal + 63, 1 ) );
	if ( !s_hunkData ) {
		Com_Error( ERR_FATAL, "Hunk data failed to allocate %i megs", s_hunkTotal / (1024*1024) );
	}

	// cacheline align
	s_hunkData = static_cast<byte *>( PADP( s_hunkData, 64 ) );
	Hunk_Clear();
...
static sysEvent_t Com_GetSystemEvent( void )
{
	...
	if ( s )
	{
		...
		len = strlen( s ) + 1;
		b = static_cast<char *>( COM_Z_MALLOC( len ) );
		strcpy( b, s );
...
static sysEvent_t Com_GetRealEvent( void ) {
	...
	if ( com_journalFile != FS_INVALID_HANDLE ) {
		...
		if ( com_journal->integer == 2 ) {
			...
			if ( ev.evPtrLength ) {
				ev.evPtr = COM_Z_MALLOC( ev.evPtrLength );
