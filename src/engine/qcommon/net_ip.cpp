...
static qboolean NET_GetPacket( netadr_t *net_from, msg_t *net_message, const fd_set *fdr )
{
	...
	{
		fromlen = sizeof(from);
		ret = recvfrom( ip_socket, reinterpret_cast<char *>( net_message->data ), net_message->maxsize, 0, reinterpret_cast<struct sockaddr *>( &from ), &fromlen );
	...
	{
		fromlen = sizeof(from);
		ret = recvfrom( ip6_socket, reinterpret_cast<char *>( net_message->data ), net_message->maxsize, 0, reinterpret_cast<struct sockaddr *>( &from ), &fromlen );
	...
	{
		fromlen = sizeof(from);
		ret = recvfrom( multicast6_socket, reinterpret_cast<char *>( net_message->data ), net_message->maxsize, 0, reinterpret_cast<struct sockaddr *>( &from ), &fromlen );
...
void Sys_SendPacket( int length, const void *data, const netadr_t *to ) {
	...
	if ( usingSocks && to->type == NA_IP ) {
		...
		if ( length <= sizeof( cmd.s.u.v4.data ) ) {
			...
			ret = sendto( ip_socket, reinterpret_cast<const char *>( cmd.buf ), length + 10, 0, reinterpret_cast<const struct sockaddr *>( &socksRelayAddr.v4 ), sizeof( socksRelayAddr.v4 ) );
		}
	}
	else {
		if ( addr.ss.ss_family == AF_INET )
			ret = sendto( ip_socket, static_cast<const char *>( data ), length, 0, reinterpret_cast<const struct sockaddr *>( &addr ), sizeof( struct sockaddr_in ) );
#ifdef USE_IPV6
		else if ( addr.ss.ss_family == AF_INET6 )
			ret = sendto( ip6_socket, static_cast<const char *>( data ), length, 0, reinterpret_cast<const struct sockaddr *>( &addr ), sizeof( struct sockaddr_in6 ) );
#endif
	}
...
static SOCKET NET_IPSocket( const char *net_interface, int port, int *err ) {
	...
	*err = 0;
	...
	if( bind( newsocket, reinterpret_cast<const struct sockaddr *>( &address ), sizeof( address ) ) == SOCKET_ERROR ) {
...
static void NET_OpenSocks( int port ) {
	...
	// get the response
	len = recv( socks_socket, reinterpret_cast<char *>( buf ), 32, 0 );
	if ( len == SOCKET_ERROR ) {
		Com_Printf( "%s: recv: %s\n", __func__, NET_ErrorString() );
		return;
	}
	...
	if ( send( socks_socket, reinterpret_cast<const char *>( &cmd ), 10, 0 ) == SOCKET_ERROR ) {
		Com_Printf( "%s: send: %s\n", __func__, NET_ErrorString() );
		return;
	}

	// get the response
	len = recv( socks_socket, reinterpret_cast<char *>( &cmd ), sizeof( cmd ), 0 );
	if ( len == SOCKET_ERROR ) {
		Com_Printf( "%s: recv: %s\n", __func__, NET_ErrorString() );
		return;
	}
