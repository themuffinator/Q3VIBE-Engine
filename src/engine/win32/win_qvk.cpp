...
qboolean VK_CreateSurface( VkInstance instance, VkSurfaceKHR *pSurface )
{
	...
	qvkCreateWin32SurfaceKHR = reinterpret_cast<PFN_vkCreateWin32SurfaceKHR>( VK_GetInstanceProcAddr( instance, "vkCreateWin32SurfaceKHR" ) );
...
static HINSTANCE load_vulkan_library( const char *dllname )
{
	...
	lib = static_cast<HINSTANCE>( Sys_LoadLibrary( dllname ) );

	if ( lib )
	{
		qvkGetInstanceProcAddr = reinterpret_cast<PFN_vkGetInstanceProcAddr>( Sys_LoadFunction( lib, "vkGetInstanceProcAddr" ) );
