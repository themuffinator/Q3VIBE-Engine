...
qboolean QGL_Init( const char *dllname )
{
	...
	if ( glw_state.OpenGLLib == NULL )
	{
		...
		glw_state.OpenGLLib = static_cast<HINSTANCE>( Sys_LoadLibrary( libName ) );
		if ( glw_state.OpenGLLib == NULL )
		{
...
#define GLE( ret, name, ... ) q##name = reinterpret_cast<ret ( APIENTRY * )( __VA_ARGS__ )>( GL_GetProcAddress( XSTRING( name ) ) ); if ( !q##name ) { Com_Printf( "Error resolving core Win32 functions\n" ); return qfalse; }
	QGL_Win32_PROCS;
#undef GLE
