...
static DWORD WINAPI ThreadProc( HANDLE hInited )
{
	...
	if ( hAVRT )
	{
		pAvSetMmThreadCharacteristicsW = reinterpret_cast<HANDLE ( WINAPI * )( LPCWSTR, LPDWORD )>( GetProcAddress( hAVRT, "AvSetMmThreadCharacteristicsW" ) );
		pAvRevertMmThreadCharacteristics = reinterpret_cast<BOOL ( WINAPI * )( HANDLE )>( GetProcAddress( hAVRT, "AvRevertMmThreadCharacteristics" ) );
...
void SNDDMA_BeginPainting( void ) {
	...
	while ((hresult = pDSBuf->lpVtbl->Lock(pDSBuf, 0, gSndBufSize, reinterpret_cast<LPVOID *>( &pbuf ), &locksize,
