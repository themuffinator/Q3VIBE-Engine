...
model_t *R_AllocModel( void ) {
	...
	mod = RI_HUNK_ALLOC_CALL( sizeof( *tr.models[tr.numModels] ), h_low );
...
static qboolean R_LoadMD3(model_t * mod, int lod, void *buffer, int bufferSize, const char *modName)
{
	...
	mod->dataSize += size;
	mdvModel = mod->mdv[lod] = RI_HUNK_ALLOC_CALL(sizeof(mdvModel_t), h_low);
...
//  Com_Memcpy(mod->md3[lod], buffer, LittleLong(md3Model->ofsEnd));
	...
	mdvModel->frames = frame = RI_HUNK_ALLOC_CALL(sizeof(*frame) * md3Model->numFrames, h_low);
	...
	mdvModel->tags = tag = RI_HUNK_ALLOC_CALL(sizeof(*tag) * (md3Model->numTags * md3Model->numFrames), h_low);
	...
	mdvModel->tagNames = tagName = RI_HUNK_ALLOC_CALL(sizeof(*tagName) * (md3Model->numTags), h_low);
	...
	mdvModel->surfaces = surf = RI_HUNK_ALLOC_CALL(sizeof(*surf) * md3Model->numSurfaces, h_low);
	...
	for(i = 0; i < md3Model->numSurfaces; i++)
	{
		...
		surf->shaderIndexes = shaderIndex = RI_HUNK_ALLOC_CALL(sizeof(*shaderIndex) * md3Surf->numShaders, h_low);
		...
		surf->indexes = tri = RI_HUNK_ALLOC_CALL(sizeof(*tri) * 3 * md3Surf->numTriangles, h_low);
		...
		surf->verts = v = RI_HUNK_ALLOC_CALL(sizeof(*v) * (md3Surf->numVerts * md3Surf->numFrames), h_low);
		...
		// swap all the ST
		surf->st = st = RI_HUNK_ALLOC_CALL(sizeof(*st) * md3Surf->numVerts, h_low);
...

	{
		...
		mdvModel->vaoSurfaces = RI_HUNK_ALLOC_CALL(sizeof(*mdvModel->vaoSurfaces) * mdvModel->numSurfaces, h_low);
...
static qboolean R_LoadMDR( model_t *mod, void *buffer, int filesize, const char *mod_name ) 
{
	...
	mod->dataSize += size;
	mod->modelData = mdr = RI_HUNK_ALLOC_CALL( size, h_low );
