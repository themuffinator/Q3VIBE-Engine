...
static shader_t *GeneratePermanentShader( void ) {
	...
	newShader = RI_HUNK_ALLOC_CALL( sizeof( shader_t ), h_low );
	...
	*newShader = shader;
	...
	for ( i = 0 ; i < newShader->numUnfoggedPasses ; i++ ) {
		...
		newShader->stages[i] = RI_HUNK_ALLOC_CALL( sizeof( stages[i] ), h_low );
		*newShader->stages[i] = stages[i];

		for ( b = 0 ; b < NUM_TEXTURE_BUNDLES ; b++ ) {
			size = newShader->stages[i]->bundle[b].numTexMods * sizeof( texModInfo_t );
			if ( size ) {
				newShader->stages[i]->bundle[b].texMods = RI_HUNK_ALLOC_CALL( size, h_low );
...
static void ScanAndLoadShaderFiles( void )
{
	...
	s_shaderText = RI_HUNK_ALLOC_CALL( sum + numMtrFiles*2 + numShaderFiles*2 + 1, h_low );
	...
	hashMem = RI_HUNK_ALLOC_CALL( size * sizeof(char *), h_low );
