...
static	void R_LoadLightmaps( const lump_t *l, const lump_t *surfs ) {
	...
	tr.lightmaps = RI_HUNK_ALLOC_CALL( tr.numLightmaps * sizeof(image_t *), h_low );

	if (tr.worldDeluxeMapping)
		tr.deluxemaps = RI_HUNK_ALLOC_CALL( tr.numLightmaps * sizeof(image_t *), h_low );
...
static	void R_LoadVisibility( const lump_t *l ) {
	...
	if ( tr.externalVisData ) {
		...
		dest = RI_HUNK_ALLOC_CALL( len - 8, h_low );
...
static void ParseFace( const dsurface_t *ds, const drawVert_t *verts, float *hdrVertColors, msurface_t *surf, int *indexes  ) {
	...
	//cv = RI_HUNK_ALLOC_CALL(sizeof(*cv), h_low);
	cv = (void *)surf->data;
	cv->surfaceType = SF_FACE;

	cv->numIndexes = numIndexes;
	cv->indexes = RI_HUNK_ALLOC_CALL(numIndexes * sizeof(cv->indexes[0]), h_low);

	cv->numVerts = numVerts;
	cv->verts = RI_HUNK_ALLOC_CALL(numVerts * sizeof(cv->verts[0]), h_low);
...
static void ParseFlare( const dsurface_t *ds, const drawVert_t *verts, msurface_t *surf, int *indexes ) {
	...
	//flare = RI_HUNK_ALLOC_CALL( sizeof( *flare ), h_low );
...
static void R_MovePatchSurfacesToHunk(void) {
	...
	for ( i = 0; i < s_worldData.numsurfaces; i++ ) {
		...
		grid->widthLodError = RI_HUNK_ALLOC_CALL( grid->width * 4, h_low );
		Com_Memcpy(grid->widthLodError, copyFrom, grid->width * 4);
		ri.Free(copyFrom);

		copyFrom = grid->heightLodError;
		grid->heightLodError = RI_HUNK_ALLOC_CALL(grid->height * 4, h_low);
		Com_Memcpy(grid->heightLodError, copyFrom, grid->height * 4);
		ri.Free(copyFrom);

		copyFrom = grid->indexes;
		grid->indexes = RI_HUNK_ALLOC_CALL(grid->numIndexes * sizeof(glIndex_t), h_low);
		Com_Memcpy(grid->indexes, copyFrom, grid->numIndexes * sizeof(glIndex_t));
		ri.Free(copyFrom);

		copyFrom = grid->verts;
		grid->verts = RI_HUNK_ALLOC_CALL(grid->numVerts * sizeof(srfVert_t), h_low);
...
static	void R_LoadSurfaces( const lump_t *surfs, const lump_t *verts, const lump_t *indexLump ) {
	...
	for ( i = 0 ; i < count ; i++, in++, out++ ) {
		switch ( LittleLong( in->surfaceType ) ) {
			case MST_PATCH:
				out->data = RI_HUNK_ALLOC_CALL( sizeof(srfBspSurface_t), h_low);
				break;
			...
			case MST_PLANAR:
				...
				break;
			case MST_FLARE:
				out->data = RI_HUNK_ALLOC_CALL( sizeof(srfFlare_t), h_low);
				break;
			default:
				break;
		}
...
static	void R_LoadSubmodels( const lump_t *l ) {
	...
	s_worldData.bmodels = out = RI_HUNK_ALLOC_CALL( count * sizeof(*out), h_low );
...
static void R_LoadLightGrid( const lump_t *l ) {
	...
	w->lightGridData = RI_HUNK_ALLOC_CALL( l->filelen, h_low );
	...
	if (r_hdr->integer)
	{
		...
		if (hdrLightGrid)
		{
			...
			w->lightGrid16 = RI_HUNK_ALLOC_CALL(sizeof(w->lightGrid16) * 6 * numGridPoints, h_low);
...
static void R_LoadEntities( const lump_t *l ) {
	...
	w->entityString = RI_HUNK_ALLOC_CALL( l->filelen + 1, h_low );
...
static void R_LoadEnvironmentJson(const char *baseName)
{
	...
	tr.cubemaps = RI_HUNK_ALLOC_CALL(tr.numCubemaps * sizeof(*tr.cubemaps), h_low);
...
void RE_LoadWorldMap( const char *name ) {
	...
	startMarker = RI_HUNK_ALLOC_CALL(0, h_low);
	c_gridVerts = 0;
	...
	s_worldData.dataSize = (byte *)RI_HUNK_ALLOC_CALL(0, h_low) - startMarker;
