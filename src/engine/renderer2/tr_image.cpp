...
static image_t *R_CreateImage2( const char *name, byte *pic, int width, int height, GLenum picFormat, int numMips, imgType_t type, imgFlags_t flags, int internalFormat ) {
	...
	image = tr.images[tr.numImages] = RI_HUNK_ALLOC_CALL( sizeof( *image ) + namelen + 1, h_low );
...
qhandle_t RE_RegisterSkin( const char *name ) {
	...
	tr.numSkins++;
	skin = RI_HUNK_ALLOC_CALL( sizeof( skin_t ), h_low );
	tr.skins[hSkin] = skin;
	...
	if ( strcmp( name + strlen( name ) - 5, ".skin" ) ) {
		skin->numSurfaces = 1;
		skin->surfaces = RI_HUNK_ALLOC_CALL( sizeof( skinSurface_t ), h_low );
	...
	skin->surfaces = RI_HUNK_ALLOC_CALL( skin->numSurfaces * sizeof( skinSurface_t ), h_low );
...
void	R_InitSkins( void ) {
	...
	skin = tr.skins[0] = RI_HUNK_ALLOC_CALL( sizeof( skin_t ), h_low );
