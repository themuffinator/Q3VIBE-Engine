...
void SND_setup( void ) 
{
	...
	if ( buffer == NULL ) {
		buffer = static_cast<sndBuffer *>( malloc( sz ) );
	}
	...
	if ( sfxScratchBuffer == NULL ) {
		sfxScratchBuffer = static_cast<short *>( malloc( sz ) );	//Hunk_Alloc(SND_CHUNK_SIZE * sizeof(short) * 4);
	}
...
//	int		size;
	...
	// load it in
	data = static_cast<byte *>( S_CodecLoad( sfx->soundName, &info ) );
	if(!data)
		return qfalse;
