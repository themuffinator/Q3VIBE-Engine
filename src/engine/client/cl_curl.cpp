...
static size_t CL_cURL_CallbackWrite( void *buffer, size_t size, size_t nmemb, void *stream )
{
	if ( clc.download == FS_INVALID_HANDLE ) {
		if ( !CL_ValidPakSignature( static_cast<const byte *>( buffer ), size*nmemb ) ) {
...
CURLcode qcurl_easy_setopt_warn(CURL *curl, long option, ...)
{
	CURLcode result;
	...
	if(option < CURLOPTTYPE_OBJECTPOINT) {
		...
		result = qcurl_easy_setopt(curl, static_cast<CURLoption>( option ), longValue);
	} else if(option < CURLOPTTYPE_OFF_T) {
		void *pointerValue = va_arg(argp, void *);
		result = qcurl_easy_setopt(curl, static_cast<CURLoption>( option ), pointerValue);
	} else {
		curl_off_t offsetValue = va_arg(argp, curl_off_t);
		result = qcurl_easy_setopt(curl, static_cast<CURLoption>( option ), offsetValue);
	}
...
static size_t Com_DL_CallbackWrite( void *ptr, size_t size, size_t nmemb, void *userdata )
{
	...
	if ( dl->fHandle == FS_INVALID_HANDLE )
	{
		if ( !CL_ValidPakSignature( static_cast<const byte *>( ptr ), size*nmemb ) ) 
		{
