...
//		{ 0.2, 1.0, 0.2, 1.0 } , { 1.0, 0.2, 0.2, 1.0 }, {0.5, 0.5, 0.5, 1} };
	...
	if ( cent->currentState.weapon && cg_weapons[ cent->currentState.weapon ].ammoModel ) {
		...
		CG_Draw3DModel( CG_CHAR_WIDTH*3 + TEXT_ICON_SPACE, y, ICON_SIZE, ICON_SIZE,
	...
	if( cg.predictedPlayerState.powerups[PW_REDFLAG] ) {
		...
		CG_DrawStatusBarFlag( 185 + CG_CHAR_WIDTH*3 + TEXT_ICON_SPACE + ICON_SIZE, TEAM_FREE );
	}
	...
	if ( ps->stats[ STAT_ARMOR ] ) {
		...
		CG_Draw3DModel( 370 + CG_CHAR_WIDTH*3 + TEXT_ICON_SPACE, y, ICON_SIZE, ICON_SIZE,
	...
	if ( cent->currentState.weapon ) {
		...
		if ( value > -1 ) {
			...
			CG_SelectFont( 1 );
			CG_DrawString( CG_CHAR_WIDTH*3, y, va( "%i", value ), colors[ color ], CG_CHAR_WIDTH, CHAR_HEIGHT, 0, DS_RIGHT | DS_PROPORTIONAL );
			CG_SelectFont( 0 );
#else
			...
			if ( !cg_draw3dIcons.integer && cg_drawIcons.integer ) {
				...
				if ( icon ) {
					CG_DrawPic( CG_CHAR_WIDTH*3 + TEXT_ICON_SPACE, y, ICON_SIZE, ICON_SIZE, icon );
				}
			}
		}
	}
...
#ifdef USE_NEW_FONT_RENDERER
	CG_SelectFont( 1 );
	CG_DrawString( 185 + CG_CHAR_WIDTH*3, y, va( "%i", value ), colors[ color ], CG_CHAR_WIDTH, CHAR_HEIGHT, 0, DS_RIGHT | DS_PROPORTIONAL );
	CG_SelectFont( 0 );
#else
	trap_R_SetColor( colors[ color ] );
	...
	if ( value > 0 ) {
#ifdef USE_NEW_FONT_RENDERER
		CG_SelectFont( 1 );
		CG_DrawString( 370 + CG_CHAR_WIDTH*3, y, va( "%i", value ), colors[ color ], CG_CHAR_WIDTH, CHAR_HEIGHT, 0, DS_RIGHT | DS_PROPORTIONAL );
		CG_SelectFont( 0 );
#else
		...
		if ( !cg_draw3dIcons.integer && cg_drawIcons.integer ) {
			CG_DrawPic( 370 + CG_CHAR_WIDTH*3 + TEXT_ICON_SPACE, y, ICON_SIZE, ICON_SIZE, cgs.media.armorIcon );
		}
	}
	...
	if( cgs.gametype == GT_HARVESTER ) {
		...
		trap_R_SetColor( colors[0] );
		CG_DrawField (640 - (CG_CHAR_WIDTH*2 + TEXT_ICON_SPACE + ICON_SIZE), y, 2, value);
		trap_R_SetColor( NULL );
		// if we didn't draw a 3D icon, draw a 2D icon for armor
		if ( !cg_draw3dIcons.integer && cg_drawIcons.integer ) {
			if( cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE ) {
...
static float CG_DrawPowerups( float y ) {
	...
	x = cgs.screenXmax + 1 - ICON_SIZE - CG_CHAR_WIDTH * 2;
	for ( i = 0 ; i < active ; i++ ) {
